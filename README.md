...
| ----------- | ------------------------- |
| WebApp URL is clicked and OAuth prompt pops up | <ul><li>Script checks if the user is already signed in.</li><li>If user is not signed in, user is asked to sign into AGO account to verify layer permissions. </li><li>Saves token to use when applying edits to layer. </li><li>If permissions are valid, WebApp loads completely. </li></ul> |
| WebApp Loads | Creates map and map view: <ul><li>Creates base layers and base layer toggle.</li><li>Creates feature layers and associated renderers. </li><li>Adds all layers to map and map view. </li><li>Creates checkboxes that allow user to toggle layers on and off. </li></ul> Finds next bridge to QC: <ul><li>Queries Bridge_Hull_QAQC feature layer for those that still have Null values for the IS_IRREG field.</li> <li>Selects one bridge from the query result at random.</li> <li>Zooms to selected bridge.</li> <li>Puts constraint on user's ability to zoom out or pan away from selected bridge.</li> <li>Highlights selected bridge.</li> <li>Saves the Object ID of the selected bridge in a global variable, for later use.</li></ul> Creates Calcite Select drop-down and Comment Box for Bridge QAQC Options: <ul><li>Select has 12 options, including "Select Your Choice" which is the default selection and does not represent an actual QAQC option.</li> <li>Comment box remains inactive until user selects an option within the irregular bridge option group.</li></ul> <p>Creates additional buttons and widgets. </p>|
| "Skip Bridge" Button is clicked | Finds next bridge to QC: <ul><li>Queries Bridge_Hull_QAQC feature layer for those that still have Null values for the IS_IRREG field.</li> <li>Selects one bridge from the query result at random.</li> <li>Zooms to selected bridge.</li> <li>Puts constraint on user's ability to zoom out or pan away from selected bridge.</li> <li>Highlights selected bridge.</li> <li>Saves the Object ID of the selected bridge in a global variable, for later use.</li></ul> Clears exisiting inputs: <ul><li> Clears any selection in the Calcite Select drop-down, and returns it to default "Select Your Choice". </li> <li> Clears any inputs in the Comment Box. </li></ul>|
| Option is selected in the Calcite Select drop-down | <ul><li> Enables Comment Box inputs.</li> <li>Enables user to click the "Save and Next Bridge" Button.</li></ul>|
| "Save and Next Bridge" Button is clicked | If user has not selected a QC option ("Select Your Choice" is still the selected option): <ul><li> An alert pops up to tell the user that there is nothing to save. </li></ul> If user has selected a QC option: <ul><li> Creates new variable to open new XML Http Request. </li> <li>Creates serviceURL variable that holds the base REST URL to update features in the Bridge_Hull_QAQC layer. </li> <li>Creates variables to save all of user's inputs (QC choice and comment) and appends these to the service URL.</li> <li>Posts edits to layer.</li></ul> Once edits get posted, finds next bridge to QC: <ul><li>Queries Bridge_Hull_QAQC feature layer for those that still have Null values for the IS_IRREG field.</li> <li>Selects one bridge from the query result at random.</li> <li>Zooms to selected bridge.</li> <li>Puts constraint on user's ability to zoom out or pan away from selected bridge.</li> <li>Highlights selected bridge.</li> <li>Saves the Object ID of the selected bridge in a global variable, for later use.</li></ul> Clears exisiting inputs: <ul><li> Clears any selection in the Calcite Select drop-down, and returns it to default "Select Your Choice". </li> <li> Clears any inputs in the Comment Box. </li></ul>|
